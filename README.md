...
3. **자동 정기 수집** — ✅ 완료(2026-08-06), ⚠️ Hobby cron 신뢰성 이슈 확인 및 백업 조치(2026-08-07). `api/auto-collect.js`가 브라우저 수집·가공 로직을 서버에서 재현해 4개 소스를 모두 수집 후 `save-snapshot`으로 저장. `vercel.json` crons에 `0 16 * * 5`(매주 금 16:00 UTC = **토 01:00 KST**) 등록, `CRON_SECRET`으로 보호.
   **2026-08-07 확인된 문제**: 첫 정식 실행일에 Vercel이 스케줄된 시각에 호출 자체를 하지 않음(설정·코드는 정상, Cron Jobs 대시보드에서도 Enabled 확인됨 — Hobby 플랜 cron이 종종 건너뛰는 것으로 보이는 신뢰성 문제). 자체 실행 로그(`/api/logs`)와 해당 날짜 스냅샷 부재로 확인함.
   **대응**: 로컬 백업 스케줄러 추가. `D:\vocational-training\analysis-notion\scripts\yongho_auto_collect_backup.bat`가 매주 **토 03:30 KST**(Windows 작업 스케줄러, 작업명 `yongho-repo\AutoCollectBackup`)에 같은 `/api/auto-collect`를 호출. 같은 날짜 스냅샷은 덮어쓰기라 Vercel cron이 정상 실행돼도 무해하게 중복됨. 인증키는 `scripts/cron_secret.txt`(git 저장소 밖, 커밋 대상 아님)에 보관 — 이 폴더를 백업/이전할 때 별도로 챙겨야 함. 이 컴퓨터가 꺼져있으면 백업도 못 돈다는 한계는 있음.
...
개발 발견 히스토리(GitHub/Vercel 연결관계 추적, 옛 클론 정리 경위)는 2026-08-05 세션, Blob Storage 연결·자동 수집·실행 로그 구현 경위는 2026-08-06 세션, Hobby cron 누락 확인 및 로컬 백업 스케줄러 구성 경위는 2026-08-07 세션 대화 기록 참조.
